...
def add_eo_index(models, schema_editor):
  ...
  graph_db.query("CREATE CONSTRAINT ON (topic:Topic) ASSERT topic.topic_uid IS UNIQUE")


def drop_eo_index(models, schema_editor):
  graph_db = graphdb_provider.get_graph_client()
  graph_db.query("DROP CONSTRAINT ON (topic:Topic) ASSERT topic.topic_uid IS UNIQUE")
