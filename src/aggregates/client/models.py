...
from src.aggregates.client.signals import created, added_ta_topic, assigned_engagement_opportunity
...
class Client(models.Model, AggregateBase):
  ...
  def _from_attrs(cls, client_name, client_type):
    ...
    ret_val._raise_event(
      created, sender=Client, instance=ret_val,
  ...
  def add_ta_topic(self, topic_type_id):
    ...
    self._raise_event(
      added_ta_topic, sender=Client, instance=self,
      ta_topic_uid=str(uuid.uuid1()), topic_type_id=topic_type_id
    )

  def assign_engagement_opportunity(self, engagement_opportunity_id):
    self._raise_event(
      assigned_engagement_opportunity, sender=Client, instance=self,
      engagement_opportunity_id=engagement_opportunity_id
    )
  ...
  def _handle_assigned_engagement_opportunity_event(self, **kwargs):
    pass
