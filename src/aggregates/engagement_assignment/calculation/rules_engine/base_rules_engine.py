...
class BaseRulesEngine(ABC):
  def apply_base_score(self, calculation_data, _iter_utils=iter_utils, _text_parser=text_parser):
    score = 0
    base_score_attrs = {}
    counter = Counter()
    ...
    stemmed_ta_topics = calculation_data[constants.STEMMED_TA_TOPICS]
    if stemmed_ta_topics:
      text_ta_topic_score = 1
      text = calculation_data[constants.TEXT]
      if text:
        text = _iter_utils.stemmify_string(text)
        for stemmed_ta_topic in stemmed_ta_topics:
          if stemmed_ta_topic in text:
            score += text_ta_topic_score
            counter[constants.TEXT_TA_TOPIC_SCORE] += text_ta_topic_score
        if counter[constants.TEXT_TA_TOPIC_SCORE]: base_score_attrs[constants.TEXT_TA_TOPIC_SCORE] = counter[
          constants.TEXT_TA_TOPIC_SCORE]

      bio = calculation_data[constants.BIO]
      if bio:
        bio_ta_topic_score = 1 * calculation_data[constants.BIO_TA_TOPIC_SCORE_MULTIPLIER]
        bio = _iter_utils.stemmify_string(bio)
        for stemmed_ta_topic in stemmed_ta_topics:
          if stemmed_ta_topic in bio:
            score += bio_ta_topic_score
            counter[constants.BIO_TA_TOPIC_SCORE] += bio_ta_topic_score

        if counter[constants.BIO_TA_TOPIC_SCORE]: base_score_attrs[constants.BIO_TA_TOPIC_SCORE] = counter[
          constants.BIO_TA_TOPIC_SCORE]
      ...
      if recent_posts:
        ...
        for stemmed_ta_topic in stemmed_ta_topics:
          for post in recent_posts:
            if stemmed_ta_topic in post:
              score += recent_post_ta_topic_score
              counter[constants.RECENT_POST_TA_TOPIC_SCORE] += recent_post_ta_topic_score
              # give, at most, 1 point per topic mention
              break

        if counter[constants.RECENT_POST_TA_TOPIC_SCORE]:
          base_score_attrs[constants.RECENT_POST_TA_TOPIC_SCORE] = counter[constants.RECENT_POST_TA_TOPIC_SCORE]

      eo_websites = calculation_data[constants.EO_WEBSITES]
      if eo_websites:
        eo_website_ta_topic_score = 1 * calculation_data[constants.EO_WEBSITE_TA_TOPIC_SCORE_MULTIPLIER]
        for eo_website in eo_websites:
          ...
          for stemmed_ta_topic in stemmed_ta_topics:
            if stemmed_ta_topic in content:
              score += eo_website_ta_topic_score
              counter[constants.EO_WEBSITE_TA_TOPIC_SCORE] += eo_website_ta_topic_score

        if counter[constants.EO_WEBSITE_TA_TOPIC_SCORE]:
          base_score_attrs[constants.EO_WEBSITE_TA_TOPIC_SCORE] = counter[constants.EO_WEBSITE_TA_TOPIC_SCORE]
