...
from src.apps.engagement_discovery.providers.twitter.services.twitter_engagement_discovery_tasks import \
  discover_engagement_opportunities_from_tweets_task
...
def discover_engagement_opportunities():
  ...
  for topic in topics_to_run:
    for subtopic in topic.subtopics.all():
      if subtopic.category_type == TopicCategoryEnum.hashtag:
        discover_engagement_opportunities_from_tweets_task.delay(subtopic.id)
